...
The draft contains `Lesson`, `Utterance`, `TurnResult`, `ObserverSessionSummary`, `StudentActionResult`, `ServiceHealth`, and `Error`, plus Core Backend, internal MI300, and local Speech Gateway APIs. Every canonical object uses `schema_version: 0.1.0`. Every API response carries `X-Request-ID`; error JSON also carries `request_id`.
...
## Observer summary and student actions

- `GET /api/sessions/{session_id}/summary` is teacher/parent-only and returns `ObserverSessionSummary`, including turn history, progress, latency/fallback data, hint history, review concepts, and familiarity.
- `POST /api/sessions/{session_id}/actions` accepts only control actions: `replay_prompt`, `start_answer`, `pause`, `resume`, `request_hint`, and `next`. It returns a student-safe `StudentActionResult`.
- `POST /api/sessions/{session_id}/turns` is the only answer-submission route and requires a non-empty `transcript`. `submit_answer` is intentionally not a control-action enum value.
- OpenAPI reuses `student-action.schema.json#/$defs/control_request` and `#/$defs/answer_submission` so generated clients cannot dispatch an answer without a transcript.
