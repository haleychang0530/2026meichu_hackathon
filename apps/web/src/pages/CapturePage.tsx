          ...
          {analysisError ? (
            <>
              <ErrorState
                ...
                onRetry={() => {
                  setAnalysisError(null);
                  setAnalysisMessage('請重新拍攝或上傳現有教材後再試。');
                }}
              />
              {import.meta.env.VITE_DATA_MODE !== 'real' ? <button className="button secondary" type="button" onClick={() => void useFixture()}>改用離線合成教材</button> : null}
            </>
          ) : null}
        </section>
